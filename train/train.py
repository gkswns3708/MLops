...
def main(config):
    ...
    def _main(config):
        ...
        train_transform, default_transform = config.init_ftn('transform_name', Custom_transforms)() 
        # TODO config.json에 trainsform_name란을 만들면 된다(type과 args를 추가)
        Dataloader = config.init_obj('Dataset', Custom_loader, train_transform=train_transform, default_transform=default_transform)
